...
export class DatabaseService
{
    ...
    /**
     * Cập nhật metadata thư viện sang bản đã dịch, đồng thời giữ lại bản gốc ở các trường original_*.
     *
     * @param source Metadata gốc đang có trong thư viện.
     * @param translated Metadata đã dịch sang tiếng Việt.
     */
    public upsertTranslatedLibraryMetadata(source: BookInfo, translated: BookInfo): void
    {
        const existing = this.db.prepare("SELECT * FROM books WHERE id = ?").get(source.bookId) as DbBookRow | undefined;
        const now = new Date().toISOString();
        ...
        this.db.prepare(
            ...
            ON CONFLICT(id) DO UPDATE SET
                title = excluded.title,
                original_title = COALESCE(books.original_title, excluded.original_title),
                author = excluded.author,
                original_author = COALESCE(books.original_author, excluded.original_author),
                description = excluded.description,
                original_description = COALESCE(books.original_description, excluded.original_description),
                cover_url = excluded.cover_url,
                chapter_count = excluded.chapter_count,
                finished = excluded.finished,
                tags_json = excluded.tags_json,
                source = COALESCE(books.source, excluded.source),
                updated_at = excluded.updated_at
            `
        ).run({
            ...
            finished: translated.finished === undefined ? null : Number(translated.finished),
            id: source.bookId,
            original_author: source.author ?? null,
            original_description: source.description ?? null,
            original_title: source.title,
            source: existing?.source ?? "legacy",
            tags_json: JSON.stringify(translated.tags ?? source.tags ?? []),
            title: translated.title,
            updated_at: now,
            cover_url: translated.coverUrl ?? null
        });
    }
